...
if(!is_file(PATH_CACHE_UBTN.$ubtn_cache_id) ) {
	...
	$b['center']['width']=$b['icon']['width']+$b['text']['width'];
	...
	if($hover){
		if ($size=='big'){
			$ink = imagecolorallocate($img,32,32,32);
			$inkh = imagecolorallocate($img,255,255,255);
		}else{
			$inkh = imagecolorallocate($img,32,32,32);
			$ink = imagecolorallocate($img,128,128,128);
		}
	}else{
		if ($size=='big'){
	...
	if(isset($b['icon']['image'])){
		$b['icon']['x']=$b['left']['width']+($b['text']['width']>0 ? $ubtn_no_text_icon_offset:0);
